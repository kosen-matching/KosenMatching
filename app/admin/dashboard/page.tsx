...
import { Loader2, CheckCircle, XCircle, AlertTriangle, Users, UserPlus, ShieldOff } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!usersRes.ok || !statsRes.ok || !imagesRes.ok) {
        const errorData = usersData || statsData || imagesData;
                    ...
                    <TableRow key={user._id}>
                      <TableCell className="font-medium">
                        <div className="flex items-center gap-2">
                          <Avatar>
                            <AvatarImage src={user.profileImageUrl ? `/api/images/${user.profileImageUrl}` : undefined} alt={user.username} />
                        ...
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                             <Button 
                                ...
                                size="sm"
                             >
                              {user.status === 'banned' ? <CheckCircle className="mr-2 h-4 w-4" /> : <ShieldOff className="mr-2 h-4 w-4" />}
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                ユーザー「{user.username}」のステータスを「{user.status === 'banned' ? 'active' : 'banned'}」に変更します。
                                {user.status !== 'banned' && " Banされたユーザーはログインできなくなります。"}
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              <AlertDialogCancel>キャンセル</AlertDialogCancel>
                              <AlertDialogAction
                                onClick={() => handleUpdateUserStatus(user._id, user.status === 'banned' ? 'active' : 'banned')}
